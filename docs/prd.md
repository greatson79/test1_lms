...
* **운영(옵션)**: 신고 접수 폼만 제공. 처리는 DB 직접 수행 (어드민 UI 없음). 메타데이터는 Seed 고정
...
2. **인증 & 온보딩**
   ...
   * 이메일 회원가입/로그인
   * 역할 선택(강사/학습자) 및 최소 프로필 (이름 필수, 휴대폰번호 선택)
   
3. **코스 상세**

   * 소개/커리큘럼/강사 정보/수강생 수/평균 평점(모의)
   * 수강신청/취소/**재신청** (취소 이력이 있는 경우 상태만 활성화, 중복 데이터 생성 방지)
...
6. **성적/피드백(학습자)**
   ...
   * 과제별 점수
   * **현재 평점** (채점된 과제 기준 가중 평균 — 퀄리티 지표)
   * **예상 최종 성적** (미제출 0점 포함 가중 평균 — 달성도 지표)
   * 두 지표를 함께 표시하여 학습자가 실제 성취도를 정확히 파악하도록 함
...
8. **코스 관리(강사)**
   ...
   * `published → draft` 역방향 허용 (수강생 존재 시 경고 모달)
   
9. **과제 관리(강사)**

   * 생성/수정/게시(draft/published/**manually_closed**). ※ 날짜에 따른 자동 마감은 시스템 상태(due_at 경과)로 처리
...
2. 코스 생성(draft) → published 전환 (실수 시 published → draft 복귀 가능)
